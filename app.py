...
def handle_image(event):
    ...
    # 先に応答してタイムアウトを防ぐ
    line_bot_api.reply_message(reply_token, TextSendMessage(text="画像を受け取りました。判定中です…"))
    ...
    if des2 is None:
        line_bot_api.push_message(user_id, TextSendMessage(text="画像の特徴が少なすぎて判定できませんでした。もう一度撮ってみてください。"))
        return
    ...
    if len(good_matches) > MATCH_THRESHOLD:
        line_bot_api.push_message(user_id, TextSendMessage(text="正解です！おめでとう！"))
    else:
        line_bot_api.push_message(user_id, TextSendMessage(text="不正解です。もう一度考えてみてね！"))
